...
import { WifiUsageListService, DataShareService, UtilityService } from '../../../_services';
...
interface UsageLog {
  ...
  bytesOut: string;
  totalUsage: string;
  allocatedUsage: string;
  datetime: string;
}

@Component({
  selector: 'app-wifi-usage-log-list',
  templateUrl: './wifi-usage-log-list.component.html',
  styleUrls: ['./wifi-usage-log-list.component.scss']
})
export class WifiUsageLogListComponent implements OnInit, AfterViewInit, OnDestroy  {

  public displayedColumns: string[] = ['username', 'name', 'mobile', 'profile', 'macAddress', 'loginTime',
    'logoutTime', 'upTime', 'bytesIn', 'bytesOut', 'totalUsage', 'allocatedUsage'];
  public data: UsageLog[] = [];
  ...
  constructor(
    private wifiUsageListService: WifiUsageListService,
  ...
  ngOnInit(): void {
    this.titleService.setTitle('WiFi Usage Log List');
    this.subscriptions.push(this.dataShareService.receivedMessage.subscribe(data => {
      ...
      this.wifiUsageListService.exportWifiUsageLogList(params)
  ...
  private getOtpLogList() {
    ...
    return this.wifiUsageListService.getWifiLogList(params);
  }
  ...
  openBottomSheet(): void {
    ...
    this.bottomSheet.open(BottomSheetFileDownloadPromptComponent, {
      data: {component: 'WifiUsageLogListComponent', items}
    });
  }
