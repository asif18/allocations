...
import { SnackbarService, YardsService } from '../../../_services';
import { Title } from '@angular/platform-browser';
import { DefaultApiResponse, DefaultListApiParams } from '../../../_models';
import * as _ from 'lodash';

declare interface Yards {
  code: number;
  name: string;
}
...
export class YardsComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  ngOnInit() {
    this.titleService.setTitle('Yards');
    this.initForm();
  }
  ...
  private getYards() {
    ...
    return this.yardsService.getYards(params);
  }
  ...
  removeYard(id: string): void {
    this.isTableLoading = true;

    const promise = this.yardsService.removeYard(btoa(id));
  ...
  onSubmit() {
    ...
    const promise = this.yardsService.saveYard(this.form.value);
